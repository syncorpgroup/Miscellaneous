...
class Order(object):
    ...
    """
    def __init__(self, store, customer, address, country=COUNTRY_PA):
        self.store = store
        ...
        self.data = {
            'Address': {'Street': self.address.street,
                        ...
                        'StreetNumber': self.address.streetnumber,
                        'UnitType': self.address.address_type,
                        'neighborhood': self.address.neighborhood,
                        'Region': self.address.country,
                        'DeliveryInstructions': self.address.deliveryinstructions,
                        'Type': self.address.address_type,
                        'Coordinates':{
                            'Latitude': self.address.latitude,
                            'Longitude': self.address.longitude
                         },
                        'OrganizationName': self.address.organizationName
                        },
...
#        item = {'Code':code, 'Qty':qty, 'ID':2, 'isNew':True, 'AutoRemove':False}
        item =  {'Code':i.code for i in self.menu.coupons if i.code == code}
        if item:
            item.update(ID=2, isNew=True, Qty=qty, AutoRemove=False)
            self.data['Coupons'].append(item)
        else:
            print("No Coupon find")
        return item
    ...
    def _send(self, url, merge):
        ...
        if self.address.country == 'COUNTRY_PA':
            print("Hola, soy de Panama")
